...
@app.get("/embeddings/", response_model=List[schemas.Embedding])
...
@app.get("/recommend/user/{user_id}", response_model=List[schemas.Recommendation])
def get_recommendation_user(user_id: int, db: Session = Depends(get_db)):
    db_user = crud.get_user(db, user_id=user_id)
    if db_user is None:
        raise HTTPException(status_code=404, detail=f"User ID {user_id} not found")
    return ml.recommend(db_user)
